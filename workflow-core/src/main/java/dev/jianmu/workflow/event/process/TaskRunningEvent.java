 ...
 * 任务开始执行事件
 *
 * <p>TaskRunningEvent表示异步任务正式开始执行时发布的事件。
 * 当任务被提交到外部执行器并开始实际执行时，
 * 系统会发布此事件。此事件继承自AsyncTaskInstanceEvent，
 * 提供了任务级别的详细信息。</p>
 *
 * <p>典型使用场景：
 * <ul>
 ...
 * <p>事件生命周期：
 * <pre>
 * TaskActivatingEvent → TaskRunningEvent (任务开始执行) → TaskSucceededEvent/TaskFailedEvent
 * </pre>
 * </p>
 *
 * @author Ethan Liu
...
public class TaskRunningEvent extends AsyncTaskInstanceEvent {
     ...
     * 事件构建器
     *
     * <p>采用Builder模式构建TaskRunningEvent实例，
     * 提供流畅的API来设置事件的各个属性。</p>
     *
     * <p>使用示例：
     * <pre>{@code
     * TaskRunningEvent event = TaskRunningEvent.Builder.aTaskRunningEvent()
    ...
    public static final class Builder {
         ...
         * 构建任务开始执行事件
         *
         * <p>使用当前Builder中设置的属性构建TaskRunningEvent实例。</p>
         *
         * @return 新的TaskRunningEvent实例
         */
